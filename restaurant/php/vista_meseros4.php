...
<div class="modal">
    <div class="modal-content">
        <h2>Menu</h2>
        <p>Elige una opción para el Menú:</p>
        <select class="seleccion" id="menu-selection">
        ...
        <button class="cancelar-modal">Cancelar</button>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.mesa-circulo').click(function() {
            ...
            mesaActiva = numeroMesa;
            mostrarElemento('vista-pedidos' + numeroMesa);
            $('#numero-mesa').text(numeroMesa);
        });

           // Manejar clics en el botón "Aceptar" del modal
            $('.modal .cerrar-modal').click(function() {
                const seleccion = $('#menu-selection');
                console.log(seleccion);
                const mesaRespuesta = `#respuesta${mesaActiva}`;
                console.log(mesaRespuesta);
                const respuestaSeleccionada = seleccion.find('option:selected').text();
                console.log(respuestaSeleccionada);
                $(mesaRespuesta).text(`Respuesta: ${respuestaSeleccionada}`);
                $('.modal').hide();
            });

            // Manejar clics en el botón "Cancelar" del modal
            $('.modal .cancelar-modal').click(function() {
                const seleccion = $('#menu-selection');
                seleccion.val('opcion1'); // Restaurar la selección por defecto
                $('.modal').hide();
            });
